...
const binanceAdapter: AnnouncementAdapter = {
  async fetchAnnouncements(): Promise<Announcement[]> {
    ...
    try {
      ...
      console.log(
        `Navigating to Binance announcements page: ${announcementUrl}`
      );
      ...
      const announcements: Announcement[] = await page.evaluate(() => {
        ...
        function logElementCount(selector: string, name: string) {
        ...
        const headlines = logElementCount(
          'h2.typography-headline5, h2[class*="typography"], .typography-headline5, h1, h2, h3, .css-1wmf07e, .css-1d1udl3, .css-zn14ny',
          "headline"
        );
          ...
          if (text && text.toLowerCase().includes("listing")) {
            ...
            if (parentSection) {
              ...
              const links = parentSection.querySelectorAll(
                'a[class*="text-Primary"], a.text-PrimaryText, a[href*="support/announcement"], a[href*="listing"], a'
              );
        ...
        const flexItems = logElementCount(
          ".bn-flex.flex-col.gap-6, .bn-flex.flex-col.gap-4, .bn-flex, [class*='card'], [class*='list-item']",
          "flex container"
        );
          ...
          links.forEach((link) => {
            ...
            if (hasKeyword) {
              ...
              const isDuplicate = results.some(
                (item) =>
                  item.title === title || (item.link === url && url !== "")
              );
      ...
      for (const link of allLinks) {
        try {
          ...
          if (!trimmedTitle || trimmedTitle.length < 10) continue;
          ...
          if (isListing) {
            ...
            const id = `binance-${Buffer.from(
              `${trimmedTitle}-${fullUrl}`
            ).toString("base64")}`;

            // Check for duplicates
            const isDuplicate = announcements.some(
              (a) => a.title === trimmedTitle || a.link === fullUrl
            );
            ...
            if (!isDuplicate) {
              ...
              console.log(
                `Added Binance announcement directly: "${trimmedTitle.substring(
                  0,
                  50
                )}..."`
              );
      ...
      await page.goto(
        "https://www.binance.com/en/support/announcement/latest-binance-listings",
        {
          waitUntil: "domcontentloaded",
          timeout: 60000,
        }
        ...
        try {
          ...
          if (isListing) {
            ...
            if (!isDuplicate) {
              ...
              console.log(
                `Added Binance announcement from second page: "${title.substring(
                  0,
                  50
                )}..."`
              );
      ...
      console.log(
        `Extracted ${announcements.length} relevant announcements from Binance`
      );
